...
public class SpecializationPage extends JPanel {
    ...
    private JPanel createButtonPanel() {
        ...
        // Style buttons if you have a helper method...
    ...
    public void refreshList() {
        ...
        List<SpecializationDAO.SpecializationDTO> specializations = specializationDAO.getSpecializationsForClinic(clinicId);
    ...
    private void addSpecialization() {
        ...
        if (newName.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Specialization name cannot be empty.", "Input Error", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (specializationDAO.addSpecialization(clinicId, newName)) {
            refreshList();
        } else {
            JOptionPane.showMessageDialog(this, "Failed to add specialization. It may already exist.", "Database Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void updateSpecialization() {
        if (selectedSpecializationId == null) {
            JOptionPane.showMessageDialog(this, "Please select a specialization from the table to update.", "No Selection", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        if (specializationDAO.updateSpecialization(selectedSpecializationId, updatedName, updatedStatus)) {
            ...
            JOptionPane.showMessageDialog(this, "Failed to update specialization. The name may already exist.", "Database Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void deleteSpecialization() {
        if (selectedSpecializationId == null) {
            JOptionPane.showMessageDialog(this, "Please select a specialization from the table to remove.", "No Selection", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            if (specializationDAO.deleteSpecialization(selectedSpecializationId)) {
                ...
                JOptionPane.showMessageDialog(this, "Failed to remove specialization. It might be in use by a doctor.", "Database Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
